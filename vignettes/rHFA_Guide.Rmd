...
al., 2019). Through identifying sites with high HFA and yield
heritability, we discuss potential strategies for selection efforts to
further incorporate local adaptation.

### How Home Field Advantage is Calculated

Overall home field advantage is conceptualized as the additional yield benefit conferred to a variety after accounting for Site; year; site-by-year; entry; home site indicator; kinship PCA axes 1-3; home site-by-kinship axes. The first step is to assign each variety a “home” site, the site where a variety performed best relative to other varieties. Where is_home is an indicator for whether site j is the home site for variety i; its coefficient is the home field advantage. We then calculate variance explained by each predictor in the final Model using the Anova function in car 3.0 (Fox & Weisberg, 2019). We then used 999 permutations to test the significance of HFA (Ewing et al., 2019). A permutation test is necessary because the HFA metric is usually positive and usually explains a significant amount of variance by traditional metrics (i.e., ANOVA), even for random data. After permuting yields within site-year, we re-assigned home site and re-calculated HFA as described above. These permuted HFAs were as expected based on variation in the dataset; we compared this with the observed HFA using a two-tailed test. A home site is assigned based on the best performance (e.g. highest yield) over the course of the time series of multi-environmental trials. This metric quantifies the benefits of environmental  specialization using “home field advantage” (HFA), which was defined as the  fitness gain an organism has in its native soil (Blanquart et al., 2013). Modern crop populations lack an obvious home location, so Ewing et al. (2019) redefined HFA as the yield advantage that a crop variety receives at its best performing, “home” location, where “best” is relative to other varieties, regardless of the location of the breeding program. This definition contrasts with a variety's origin—the target region for which it was bred.

# Guide

## Step 1: Acquire dataset

Data must be in long format containing the plant variety and data from
each location:
...
Note that this is a ***tall*** dataframe. Columns and corresponding
arguments in `rHFA` functions:
...
-   IS_CHECK: Unique to this dataset. Flag for whether a line is a
    "check" line - i.e. grown in all locations in a given year.
...
The design is somewhere between an augmented block and a common garden
experiment: a population of commercial hybrids was grown across
locations, only the "checks" were grown in each location in a given
year. We have already subsetted the dataset to include only those
checks.
...
modeling approaches, especially for sparser datasets. rHFA also provides
tools to filter the dataset to a denser subset.

### Notes about checking your data

It is important for you to make sure that you examine your data for missing values, for consistancy of names accross sites, and for consistancy of variety across site. Doing checks for outliers and potentially incorrect values is also important. 
...
This gives us `r length(levels(corn$HYBRID))` hybrids to work with, down
from `r length(unique(corn$HYBRID))`.
...
example, domestication events of dry bean), specify a column that
differentiates them.

As the function suggests, this is a permutation test. Because the HFA
algorithm first identifies sites where a genotype performs relatively
well, we expect a positive effect of growing at that site. The real
question is, is the positive effect larger than expected?
...
observed data). In practice, use *at least* 99.
...
r round(pophfa$home_field[1, 1]/mean(corn$YIELD), 3)\*100

\% increase over the average corn yield in this dataset. However, this
was about 1.3 bushels per acre less than expected (90% confidence
interval: -2.2 to -0.2 bushels per acre). Every permutation's HFA was
larger than the observed HFA (*p* = 0), which you can verify by looking
at the permutations themselves:
...
FIX THIS STATEMENT ----\> r round(ss['is_home', 'pct Var'], 2) % of
total variance. Not much.
...
Lines at home tend to perform better than average at that location
(rel_YIELD \> 0).
...
results suggest generalist populations. Still:

-   Belleville may be an environmentally distinct location, as HFA is
