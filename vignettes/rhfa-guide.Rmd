...
Now, look at results. `permute_hfa()` returns a list with four objects:

1.  `.$home_field`, which contains HFA effects and the results of a
    permutation test.
2.  `.$data`, containing the subset of data (including calculated internally)
    for the analysis
...
## Generic covariates
Suppose you think a covariate like kinship affects the strength of HFA. rHFA will allow
you to specify them in the 'covars' section. We will demonstrate with a random
variable. 
...
either characters specifying column names or a formula (handy for multiple
covariates). This uses the formula specification option. 
```{r}
